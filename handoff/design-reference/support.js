...
(() => {
    ...
    function dcNameFromPath(pathname) {
        ...
        return (
            base.replace(/\.dc\.html$/, '').replace(/\.html?$/, '') || 'Root'
        );
    }
  ...
  `;
    ...
    function resolvePath(vals, expr) {
        ...
        while (i < expr.length) {
            if (expr[i] === '.') {
                const m =
                    expr.slice(i + 1).match(IDENT_RE) ||
                    expr.slice(i + 1).match(/^\d+/);
                if (!m) return void 0;
    ...
    function compileTemplate(html, host) {
        ...
        const render = (vals, ctx) =>
            builders.map((b, i) => b(vals || {}, ctx, i));
    ...
    function warnUnresolved(ctx, what) {
        ...
        warnedHoles.add(key);
        console.warn(
            '[dc-runtime] ' + (ctx?.__name || 'template') + ': ' + what,
        );
    }
    ...
    function walkComponent(el, host) {
        const name =
            el.getAttribute('name') || el.getAttribute('component') || '';
        ...
        return (vals, ctx, key) => {
            ...
            if (kids.length)
                props.children = kids.map((b, j) => b(vals, ctx, j));
    ...
    function walkXImport(el, host) {
        ...
        const deckKeyed =
            hasContent && isDeckMountTag(el)
                ? walkDeckChildren(el, host)
                : null;
    ...
    function walkElement(el, host) {
        ...
        const deckKeyed = isDeckMountTag(el)
            ? walkDeckChildren(el, host)
            : null;
        ...
        return (vals, ctx, key) => {
            ...
            return h(
                realTag,
                props,
                ...renderDeckKids(kids, kidKeys, vals, ctx),
            );
        };
    }
    ...
    function Placeholder({ name, hintSize, streaming, error }) {
        ...
        return h(
            ...
            {
                className:
                    'sc-placeholder' + (streaming ? ' sc-streaming' : ''),
            ...
            const tick = () => {
                ...
                if (Date.now() - started >= GLOBAL_POLL_TIMEOUT_MS) {
                    console.warn(
                        ...
                        JSON.stringify(name),
                        'never appeared on window after ' +
    ...
    function createRuntime(doc = document) {
        ...
        function updateHtml(name, html) {
            ...
            try {
                ...
                console.error(
                    '[dc-runtime] template compile FAILED for',
                    name,
                    e,
                );
            }
